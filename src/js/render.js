...
function drawClusterLinks() {
  ...
  /* -------------------------------------------------------------------------
 * Raised architectural plinth for exposed side machinery.
 *
 * This is deliberately NOT a building shell.
 * It establishes ownership of the floor space and gives exposed machinery
 * a substantial architectural presence.
 *
 * The machinery remains completely visible above it.
 * ------------------------------------------------------------------------- */
function drawMachineryPlinth(o, id, opts) {
  opts = opts || {};

  var cs = casing(id);

  var padX = opts.padX !== undefined ? opts.padX : 0.55;
  var padY = opts.padY !== undefined ? opts.padY : 0.55;
  ...
  var baseH = opts.baseH !== undefined ? opts.baseH : 0.22;
  var stepH = opts.stepH !== undefined ? opts.stepH : 0.10;

  var baseColor = opts.baseColor || '#18251b';
  var edgeColor = opts.edgeColor || '#34543a';
  var deckColor = opts.deckColor || '#263c2b';
  var metalColor = opts.metalColor || '#69736d';
  ...
  Iso.box(ctx, {
    ...
    h:stepH,
    color:deckColor,
    edge:'rgba(15,22,17,0.75)'
  });

  /* ---------------------------------------------------------------
   * RAISED PERIMETER KERB
   * --------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------
   * EDGE LIGHT / IDENTITY STRIP
   *
   * Very restrained. It distinguishes the machine platform without
   * turning the plinth into a glowing platform.
   * --------------------------------------------------------------- */
...
function configWorld(o, lx, ly, lz) {
  ...
  function configBeam(x0, y0, z0, x1, y1, z1, width, color) {
    ...
    /* Horizontal run: use the normal factory pipe. */
    ...
    /* Vertical riser: pipe() only supports horizontal floor-plane runs,
       so represent a vertical configuration riser as a cylinder. */
    if (Math.abs(x1 - x0) < 0.001 && Math.abs(y1 - y0) < 0.001) {
      var zLow = Math.min(z0, z1);
      var zHigh = Math.max(z0, z1);
    ...
    /* Fallback for a genuinely 3D segment.
       The current curator uses only horizontal and vertical segments,
       but keep this safe rather than passing malformed arguments to pipe(). */
    pipe(
      x0, y0,
      x1, y1,
      z0,
...
function drawConfigCurator(o) {
  ...
  /*
   * -------------------------------------------------------------------------
   * ec-config-curator — exposed configuration-control machinery
   *
   * The service intentionally remains an OPEN MACHINE.
   *
   * There is no enclosing casing, roof or monolithic building.
   *
   * Instead, the entire assembly now sits on a substantial architectural
   * machinery plinth. The plinth establishes ownership of the floor space
   * and gives the machinery a deliberate "installation" rather than making
   * it look like objects randomly placed on the factory floor.
   *
   * The machinery itself remains visually dominant.
   * -------------------------------------------------------------------------
   */

  var cs = casing('config');
  ...
  /* -----------------------------------------------------------------------
   * 0. ARCHITECTURAL MACHINERY PLINTH
   *
   * Three visual layers:
   *
   *   factory floor
   ...
   *   heavy foundation
   *       ↓
   *   raised machinery deck
   *       ↓
   *   restrained perimeter kerb
   *
   * This is infrastructure, not a casing.
   * ----------------------------------------------------------------------- */
  ...
  var foundationH = 0.24;
  var deckH = 0.12;
  var kerbH = 0.12;
  var kerbW = 0.16;

  /* heavy lower foundation */
  Iso.box(ctx, {
    x:px,
    y:py,
    z:0,
   ...
   * Four anchor plates.
   *
   * These make the plinth read as engineered infrastructure rather than
   * simply a coloured rectangle underneath the machinery.
   */
  [
    [px + 0.46, py + 0.46],
    [px + pw - 0.46, py + 0.46],
    [px + 0.46, py + pd - 0.46],
    [px + pw - 0.46, py + pd - 0.46]
  ].forEach(function (a) {
  ...
  /*
   * Two restrained identification/status lamps.
   * These are deliberately low-key; the machine should not become a
   * glowing platform.
   */
  lamp(
    px + pw * 0.50,
   ...
   * Machinery elevation.
   *
   * Everything below this point is now built from the raised machinery
   * deck instead of the factory floor.
   */
  var floorZ = plinthZ + kerbH;
  var steelZ = floorZ + 0.12;


  /* -----------------------------------------------------------------------
   * 1. STRUCTURAL GROUNDING
   *
   * Steel rails sit ON the plinth and establish the machinery's footprint.
   * They do not form a casing.
   * ----------------------------------------------------------------------- */
   ...
   *
   * Proper hopper geometry rather than a rectangular container.
   * ----------------------------------------------------------------------- */
   ...
   *
   * The visual centrepiece of Config Curator.
   *
   * This is intentionally larger than before so that the service feels
   * like a substantial machine rather than a collection of props.
   * ----------------------------------------------------------------------- */
  ...
  for (var q = 0; q < 6; q++) {
    ...
    var qx = rotX + Math.cos(qa) * 0.66;
    var qy = rotY + Math.sin(qa) * 0.66;
  ...
  /* -----------------------------------------------------------------------
   * 5. SINGLE SHEDLOCK MECHANISM
   *
   * Small but deliberately isolated.
   * ----------------------------------------------------------------------- */
   ...
   *
   * Separate physical mechanism because replay is distinct from the
   * nine-way bootstrap distribution.
   * ----------------------------------------------------------------------- */
   ...
   * 9. MACHINE IDENTITY
   *
   * Keep this tiny. The machinery itself is the hero.
   * ----------------------------------------------------------------------- */
  ...
  /* -----------------------------------------------------------------------
   * 10. PLINTH IDENTITY MARK
   *
   * Very subtle architectural marking at the edge of the installation.
   * This is deliberately not another large floating label.
   * ----------------------------------------------------------------------- */
...
// }
   ...
   * ec-review-service
   *
   * Reviewer Entitlement Matrix
   *
   * The service does not process communications. It resolves:
   *
   *     reviewer
   *        ×
   *     pipeline membership
   *        ×
   *     entitlement / participant attributes
   *        ↓
   *     reviewer visibility
   *
   * Visually this is an open industrial authorization machine rather
   ...
   *     permitted lanes
   *
   * No shared side-service casing is used.
   */
   ...
   * 1. STRUCTURAL PLINTH
   *
   * A shallow steel foundation establishes this as a deliberate
   * machine installation rather than something sitting randomly
  ...
  Iso.box(ctx, {
    ...
    h: 0.12,
    color: '#263743',
    edge: 'rgba(160,190,205,0.28)'
  });

  /*
   * Four visible foundation anchor plates.
   */
  [
    [o.x + 0.35, o.y + 0.35],
    [o.x + W - 0.35, o.y + 0.35],
    [o.x + 0.35, o.y + D - 0.35],
    [o.x + W - 0.35, o.y + D - 0.35]
  ].forEach(function (p) {
    Iso.box(ctx, {
      x: p[0] - 0.16,
      y: p[1] - 0.16,
  ...
  /* ================================================================
   * 2. REAR AUTHORIZATION SPINE
   *
   * The large rear frame gives the machine height without becoming
   * a box. It represents the persistent reviewer/pipeline registry.
   * ================================================================ */
  ...
  Iso.box(ctx, {
    ...
    h: 2.35,
    color: dark,
    edge: 'rgba(130,165,185,0.38)'
  });

  /*
   * Vertical structural columns.
   */
  [0.65, W - 0.65].forEach(function (lx) {
    Iso.box(ctx, {
      x: o.x + lx - 0.10,
      y: rearY - 0.10,
      z: steelZ,
  ...
  /* ================================================================
   * 3. REVIEWER BADGE INPUT RAIL
   *
   * Reviewer groups enter from the left side as physical badges.
   * ================================================================ */
  ...
  stencil(
    ...
    0.55,
    'REVIEWERS',
    { size: 3.4, color: 'rgba(205,225,235,0.62)' }
  );

  /*
   * Four physical reviewer/group badges.
   */
  for (var bi = 0; bi < 4; bi++) {
    var by = o.y + 1.45 + bi * 0.55;
    ...
    Iso.box(ctx, {
      ...
      d: 0.26,
      h: 0.10,
      color: bi % 2 === 0 ? blue : steel,
      edge: 'rgba(220,240,245,0.35)'
    });
   ...
   *
   * Rows = reviewer/pipeline relationships.
   * Columns = entitlement dimensions / attributes.
   *
   * Each node is an actual mechanical switching point.
   * ================================================================ */

  var matrixX = o.x + 1.55;
  var matrixY = o.y + 0.92;
  ...
  Iso.box(ctx, {
    ...
    h: 0.18,
    color: dark,
    edge: 'rgba(120,160,185,0.30)'
  });

  /*
   * Raised perimeter rails.
   */
  Iso.box(ctx, {
    x: matrixX,
    y: matrixY,
  ...
  var rowGap = matrixD / (rowsM + 1);
  var colGap = matrixW / (colsM + 1);

  /*
   * Horizontal reviewer/pipeline rails.
   */
  for (var r = 0; r < rowsM; r++) {
    var ry = matrixY + rowGap * (r + 1);

    configBeam(
      matrixX + 0.18,
      ry,
      0.66,
      matrixX + matrixW - 0.18,
      ry,
      0.66,
      0.075,
      r === 1 ? blue : '#657784'
    ...
    floorText(
      ...
      {
        size: 3.8,
        color: 'rgba(215,225,232,0.72)'
      }
    );
  }

  /*
   * Vertical entitlement rails.
   */
  for (var c = 0; c < colsM; c++) {
    var rx = matrixX + colGap * (c + 1);
     ...
     * Attribute labels.
     */
    var attrNames = ['GROUP', 'QUEUE', 'ENTITY', 'REGION', 'ROLE'];

    floorText(
      rx - 0.34,
      matrixY + matrixD + 0.32,
      0.50,
      [attrNames[c]],
      {
        size: 3.0,
   ...
   * Switching nodes.
   *
   * These are deliberately varied rather than being a perfect
   * decorative grid. The pattern represents some reviewer
   * entitlements existing and others not existing.
   */
  var nodePattern = [
    [1, 1, 0, 1, 0],
    [0, 1, 1, 1, 1],
  ...
  for (var rr = 0; rr < rowsM; rr++) {
    for (var cc = 0; cc < colsM; cc++) {
      var nx = matrixX + colGap * (cc + 1);
      var ny = matrixY + rowGap * (rr + 1);
      var enabled = nodePattern[rr][cc] === 1;

      /*
       * Mechanical relay body.
       */
      Iso.cylinder(ctx, {
        x: nx,
        y: ny,
       ...
       * Tiny switching arm.
       */
      var armX = nx + (enabled ? 0.18 : -0.18);

      Iso.box(ctx, {
        x: Math.min(nx, armX),
        y: ny - 0.025,
        z: 0.99,
        w: Math.abs(armX - nx) || 0.04,
        d: 0.05,
        h: 0.045,
        color: enabled ? amber : '#4b565d'
      });
    }
  }

  /* ================================================================
   * 5. CENTRAL CONTROL BUS
   *
   * A physical bus connects the matrix to the authorization output.
   * ================================================================ */
  ...
  Iso.box(ctx, {
    ...
    h: 0.16,
    color: dark2,
    edge: 'rgba(130,170,185,0.35)'
  });

  /*
   * Five small active bus indicators.
   */
  for (var lb = 0; lb < 5; lb++) {
    var bx = o.x + 1.65 + lb * ((W - 2.25) / 4);
  ...
  stencil(
    ...
    {
      size: 3.5,
      color: 'rgba(170,210,225,0.58)'
    }
  );

  /* ================================================================
   * 6. POPULATION / ATTRIBUTE SCANNER
   *
   * This represents the validation of entitlement rows against the
   * surveilled population and alert participant attributes.
   * ================================================================ */

  var scanX = o.x + W - 0.90;
  var scanY = o.y + 1.05;
  ...
  /* ================================================================
   * 7. OUTPUT AUTHORIZATION GATE
   *
   * The machine produces permitted reviewer visibility rather than
   * producing a Kafka message.
   * ================================================================ */

  var gateX = o.x + W - 1.15;
  var gateY = o.y + D - 0.48;
  ...
  floorText(
    ...
    {
      size: 3.4,
      color: 'rgba(125,215,155,0.72)'
    }
  );

  /* ================================================================
   * 8. ALCASTRAZ / MONGODB REGISTRY
   *
   * The backing database is explicitly named alcatraz in the source,
   * so retain that as a small physical registry cabinet rather than
   * making the whole machine look like a database server.
   * ================================================================ */
  ...
  stencil(
    ...
    {
      size: 3.2,
      color: 'rgba(210,220,225,0.62)'
    }
  );

  /* ================================================================
   * 9. CDC / WHOLE-REPLACE MECHANISM
   *
   * Pipeline structure is synchronized by CDC and replaced wholesale,
   * not merged. Make that visible as a physical cassette exchange.
   * ================================================================ */

  var cassetteX = o.x + W * 0.48;
  var cassetteY = o.y + 0.35;
  ...
  floorText(
    ...
    {
      size: 3.1,
      color: 'rgba(220,125,105,0.65)'
    }
  );

  /* ================================================================
   * 10. QUERY / REST CONNECTION HATCHES
   *
   * Two callers:
   *   portal
   *   external-api
  ...
  var hatch1X = o.x + 1.00;
  var hatch2X = o.x + W - 2.00;

  [hatch1X, hatch2X].forEach(function (hx, hi) {
    Iso.cylinder(ctx, {
      x: hx,
      y: sf + 0.04,
      z: 0.52,
    ...
    stencil(
      ...
      {
        size: 3.0,
        color: 'rgba(180,215,230,0.58)'
      }
    );
  });

  /* ================================================================
   * 11. PHYSICAL REVIEW AUTHORITY HMI
   * ================================================================ */
  ...
  for (var hb = 0; hb < 7; hb++) {
    Iso.box(ctx, {
      ...
      d:0.018,
      h:0.045 + (hb % 3) * 0.025,
      color:hb < 5 ? '#6fc487' : '#c69b48'
    });
  }

  /* physical control fascia */
  Iso.box(ctx, {
    x:hmiX - 0.76,
    y:hmiY - 0.34,
    z:hmiZ + 0.38,
  ...
  /* ================================================================
   * 12. SAFETY / INFORMATION STENCILS
   * ================================================================ */
  ...
  stencil(
    ...
    {
      size: 3.5,
      color: 'rgba(170,205,220,0.48)'
    }
  );

  /*
   * Fast-fail indicator for malformed CDC payload.
   * The real service sends malformed CDC events directly to DLT
   * rather than retrying.
   */
  var failX = o.x + W - 0.42;
  var failY = o.y + 0.48;
  ...
  floorText(
    ...
    {
      size: 3.0,
      color: 'rgba(220,105,95,0.60)'
    }
  );

  /*
   * Ambient mechanical details.
   */
  for (var rib = 0; rib < 5; rib++) {
    var ribX = o.x + 1.55 + rib * ((W - 3.15) / 4);
   ...
   *
   * TIERED ACTION EXECUTION PLANT
   *
   * Four incoming action tiers feed a central execution mechanism.
   * The action is checked against hold/interlock state and then
   * independently applied to:
   *
   *     supervised_item  -> MongoDB
   *     review index     -> Elasticsearch
   *     metrics          -> rollup
   *     event log        -> audit/event stream
   *
   * The Mongo and ES operations remain physically independent.
   * ---------------------------------------------------------------
   */
  ...
  var steel = '#687a82';
  var steelDark = '#34434a';
  var steelLight = '#87979c';

  var dark = '#1d272c';
  var dark2 = '#26343a';

  var action = '#8a5362';
  var actionBright = '#c47a8b';
  ...
  tiers.forEach(function(t, i) {
    ...
    stencil(
      ...
      {
        size:4.0,
        color:'rgba(215,225,228,0.78)'
      }
    );

    /*
     * physical downward feed chute
     */
    Iso.box(ctx, {
      x:t.x + 0.42,
      y:ty + 0.62,
  ...
  /*
   * ===============================================================
   * 3. CENTRAL ACTIONING FRAME
   * ===============================================================
   */

  var frameX = o.x + W * 0.50;
  var frameY = o.y + D * 0.48;

  /*
   * rear structural columns
   */
  [o.x + 0.55, o.x + W - 0.55].forEach(function(px) {
  ...
  /*
   * ===============================================================
   * 4. CENTRAL ACTIONING PRESS
   * ===============================================================
   */

  var pressX = frameX;
  var pressY = o.y + D * 0.54;
  ...
  Iso.box(ctx, {
    ...
    h:0.78,
    color:dark,
    edge:'rgba(170,195,205,0.35)'
  });

  /*
   * glowing action chamber face
   */
  Iso.box(ctx, {
    x:pressX - 0.82,
    y:pressY - 0.62,
  ...
  /*
   * ===============================================================
   * 5. HOLD / LEGAL-HOLD INTERLOCK
   * ===============================================================
   */

  var lockX = pressX;
  var lockY = pressY + 0.72;
  ...
  stencil(
    ...
    {
      size:3.0,
      color:'rgba(215,210,175,0.55)'
    }
  );

  /*
   * ===============================================================
   * 6. TWO INDEPENDENT WRITE RAMS
   *
   * The physical separation is intentional.
   * ===============================================================
   */

  var writeY = o.y + D - 0.70;
  ...
  writes.forEach(function(wr) {
    ...
    Iso.box(ctx, {
      ...
      d:0.34,
      h:0.18,
      color:Iso.mix(wr.color, steelDark, 0.32)
    });
  ...
  /*
   * ===============================================================
   * 7. METRICS ROLLUP MECHANISM
   * ===============================================================
   */

  var metricsX = o.x + W * 0.50;
  var metricsY = o.y + D - 0.05;
  ...
  stencil(
    ...
    {
      size:3.1,
      color:'rgba(165,205,220,0.62)'
    }
  );

  /*
   * ===============================================================
   * 9. NON-RETRYABLE FAILURE CHUTE
   *
   * There is no retry topic / DLT.
   * This chute therefore physically dead-ends.
   * ===============================================================
   */
  ...
  stencil(
    ...
    {
      size:3.4,
      color:'rgba(220,110,100,0.72)'
    }
  );

  /*
   * ===============================================================
   * 10. ITEM-NOT-FOUND REJECT
   * ===============================================================
   */

  var rejectX = o.x + 0.42;
  var rejectY = o.y + D + 0.28;
  ...
  /*
   * ===============================================================
   * 12. MECHANICAL DETAIL
   * ===============================================================
   */

  /*
   * floor rails beneath the machine
   */
  ...
  /*
   * warning lamps across upper frame
   */
  for (var wl = 0; wl < 5; wl++) {

    lamp(
      o.x + 1.05 + wl * ((W - 2.10) / 4),
      o.y + 1.00,
      top - 0.02,
      0.045,
      true,
